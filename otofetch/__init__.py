"""
Init module for otofetch. This module contains the main entry point for otofetch.
And OtoFetch class
"""
...
from otofetch.download.downloader import Downloader
from otofetch.types.options import DownloaderOptionalOptions, DownloaderOptions
from otofetch.types.song import Song
from otofetch.utils.search import parse_query
from otofetch.utils.spotify import SpotifyClient

__all__ = ["OtoFetch", "console_entry_point", "__version__"]
...
class OtoFetch:
    """
    OtoFetch class, which simplifies the process of downloading songs from Spotify.

    ```python
    from otofetch import OtoFetch

    client = OtoFetch(client_id='your-client-id', client_secret='your-client-secret')

    songs = client.search(['joji - test drive',
        'https://open.spotify.com/track/4cOdK2wGLETKBW3PvgPWqT'])

    results = client.download_songs(songs)
    song, path = client.download(songs[0])
    ```
    """
