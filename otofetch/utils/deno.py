...
from otofetch.utils.config import get_otofetch_path
...
def get_local_deno() -> Optional[Path]:
    ...
    deno_path = Path(get_otofetch_path()) / (
...
def get_local_deno_yt_dlp_options() -> Dict[str, Dict[str, Dict[str, str]]]:
    """
    Get yt-dlp options that point to OtoFetch's local Deno binary.
...
def warn_if_deno_missing() -> None:
    ...
    logger.warning(
        "Some YouTube downloads require Deno. Run otofetch --download-deno "
...
def download_deno() -> Path:
    """
    Download Deno binary to otofetch directory.
    ...
    deno_path = Path(get_otofetch_path()) / deno_filename
