...
Default config - otofetch.utils.config.DEFAULT_CONFIG
"""
...
from otofetch.types.options import (
    DownloaderOptions,
    OtoFetchOptions,
    SpotifyOptions,
)
...
def get_otofetch_path() -> Path:
    """
    Get the path to the otofetch folder, following XDG standards on Linux.
    ~/.config/otofetch/ is used if it exists, else ~/.otofetch if it exists.
    If the otofetch directory does not exist, it will be created

    ### Returns
    - The path to the otofetch folder.
    ...
    if platform.system() == "Linux":
        # Define the new, correct XDG config path (~/.config/otofetch)
        xdg_config_path = Path.home() / ".config" / "otofetch"

        # Define the old path (~/.otofetch) for backward compatibility
        old_otofetch_path = Path.home() / ".otofetch"
        ...
        if old_otofetch_path.exists():
            return old_otofetch_path

        # Scenario 3: The user is brand new. Create and use the new XDG path.
        os.makedirs(xdg_config_path, exist_ok=True)
        return xdg_config_path

    # For non-Linux systems (like Windows), use the default ~/.otofetch path
    otofetch_path = Path.home() / ".otofetch"
    os.makedirs(otofetch_path, exist_ok=True)

    return otofetch_path
...
def get_config_file() -> Path:
    ...
    return get_otofetch_path() / "config.json"
...
def get_cache_path() -> Path:
    ...
    return get_otofetch_path() / ".spotipy"
...
def get_spotify_cache_path() -> Path:
    ...
    return get_otofetch_path() / ".spotify_cache"
...
def get_temp_path() -> Path:
    ...
    temp_path = get_otofetch_path() / "temp"
...
def get_errors_path() -> Path:
    ...
    errors_path = get_otofetch_path() / "errors"
...
def get_config() -> Dict[str, Any]:
    ...
    if not config_path.exists():
        raise ConfigError(
            ...
            "Please run `otofetch --generate-config` to create a config file."
        )
...
def create_settings(
    arguments: Namespace,
) -> Tuple[SpotifyOptions, DownloaderOptions]:
    """
    Create settings dicts for Spotify and Downloader
...
DEFAULT_CONFIG: OtoFetchOptions = {
