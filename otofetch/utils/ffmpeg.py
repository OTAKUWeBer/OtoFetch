...
from otofetch.utils.formatter import to_ms
...
def get_local_ffmpeg() -> Optional[Path]:
    ...
    ffmpeg_path = Path(get_otofetch_path()) / (
...
def download_ffmpeg() -> Path:
    """
    Download ffmpeg binary to otofetch directory.
    ...
    ffmpeg_path = Path(
        os.path.join(
            get_otofetch_path(), "ffmpeg" + (".exe" if os_name == "windows" else "")
        )
    )

    # Download binary and save it to a file in otofetch directory
