...
from otofetch.download.downloader import AUDIO_PROVIDERS, LYRICS_PROVIDERS
from otofetch.utils.ffmpeg import FFMPEG_FORMATS
from otofetch.utils.formatter import VARS
from otofetch.utils.logging import NAME_TO_LEVEL

__all__ = ["OPERATIONS", "SmartFormatter", "parse_arguments"]

OPERATIONS = ["download", "save", "sync", "meta", "url"]
...
def parse_main_options(parser: _ArgumentGroup):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "It's located under C:\\Users\\user\\.otofetch\\config.json "
            "or ~/.otofetch/config.json under linux"
        ),
    )
...
def parse_spotify_options(parser: _ArgumentGroup):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "It's located under C:\\Users\\user\\.otofetch\\.spotify_cache "
            "or ~/.otofetch/.spotify_cache under linux. "
            "It only caches tracks and "
            "gets updated whenever OtoFetch gets metadata from Spotify. "
...
def parse_other_options(parser: _ArgumentGroup):
    ...
    parser.add_argument(
        "--download-ffmpeg",
        action="store_true",
        help="Download ffmpeg to otofetch directory.",
    )

    parser.add_argument(
        "--download-deno",
        action="store_true",
        help="Download Deno to otofetch directory.",
    )
